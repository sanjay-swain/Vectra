...
use crate::{
    dynamics::{
        constraint_solver::ConstraintSolver, forces::compute_resultant, gauss_seidel::gauss_sediel,
    },
    system::{
        body::Body,
        constraints::{
...
impl ConstraintSolver for SoftConstraint {
    fn solve(&self, constraint: &mut Constraint, bodies: &Vec<Body>) {
        ...
        let alpha = 2.0 / self.tau;
        let beta = 2.0 / (self.tau * self.tau);

        let c = constraint.joint.calculate_joint_error(
        ...
        let c_dot = constraint.joint.calculate_joint_velocity_error(
            &bodies[constraint.body_a_index].state,
            &bodies[constraint.body_b_index].state,
            constraint.anchor_a,
            constraint.anchor_b,
        ...
        for i in 0..n {
            ...
            for j in 0..n {
                ...
                if j == i {
                    k_matrix[i][j] = k_matrix[i][j] + self.epsilon;
                }
            }
